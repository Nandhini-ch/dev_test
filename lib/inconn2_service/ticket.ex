defmodule Inconn2Service.Ticket do
  ...
  def list_category_helpdesks(prefix) do
    Repo.all(CategoryHelpdesk, prefix: prefix) |> Repo.preload([:site, :workrequest_category])
  end
  ...
  """
  def get_category_helpdesk!(id, prefix), do: Repo.get!(CategoryHelpdesk, id, prefix: prefix) |> Repo.preload([:site, :workrequest_category])
  ...
  def create_category_helpdesk(attrs \\ %{}, prefix) do
    result = %CategoryHelpdesk{}
              ...
              |> Repo.insert(prefix: prefix)
    case result do
      {:ok, category_helpdesk} -> {:ok, category_helpdesk |> Repo.preload([:site, :workrequest_category])}
       _ -> result
    end
  end
  ...
  def update_category_helpdesk(%CategoryHelpdesk{} = category_helpdesk, attrs, prefix) do
    result = category_helpdesk
              ...
              |> Repo.update(prefix: prefix)
    case result do
      {:ok, category_helpdesk} -> {:ok, category_helpdesk |> Repo.preload([:site, :workrequest_category], force: true)}
      _ -> result
    end
  end
