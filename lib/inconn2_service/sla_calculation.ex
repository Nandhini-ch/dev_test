defmodule Inconn2Service.SlaCalculation do
  ...
  def get_scope_details_from_contract(contract_id, prefix) do
    ...
    final_map = Enum.reduce(scopes, final_map, fn scope, acc -> Map.put(acc, :location_ids, get_location_of_scope(scope, prefix) ++ acc.location_ids)  end)

    Enum.reduce(scopes, final_map, fn scope, acc -> Map.put(acc, :asset_category_ids, get_asset_category_of_scope(scope, prefix) ++ acc.asset_category_ids)  end)
  end
  ...
  def get_mtbf_status(contract_id, from_date, to_date, prefix) do
    ...
    from(e in Equipment, where: e.asset_category_id in ^scope_map.asset_category_ids and e.location_id in ^scope_map.location_ids and e.site_id in ^scope_map.site_ids,
    select: e.id)
  ...
  def shift_coverage(contract_id, from_date, to_date, prefix) do
    ...
    manpower_list =
      from(mr in MasterRoster, where: mr.site_id in ^scope_map.site_ids,
      join: r in Roster, on: r.master_roster_id == mr.id, where: ^from_dt <= r.date and ^to_dt >= r.date,
      select: r
      ) |> Repo.all(prefix: prefix)

    count_of_manpower_present_in_shifts =
      from(at in Attendance, where: at.site_id in ^scope_map.site_ids and not is_nil(at.shift_id) and ^from_dt <= at.out_time and ^to_dt >= at.out_time)
      |> Repo.all(prefix: prefix)

    calculate_percentage(Enum.count(count_of_manpower_present_in_shifts), Enum.count(manpower_list))
  end
  ...
  def deployment_status(contract_id, from_date, to_date, prefix) do
    ...
    total_manpower_as_per_service_contract_setup =
      from(mp in ManpowerConfiguration, where: mp.site_id in ^scope_map.site_ids and mp.contract_id == ^contract_id)
      |> Repo.all(prefix: prefix)
  ...
  def ticket_completion(contract_id, from_date, to_date, prefix) do
    ...
    ticket_list =
      from(wr in WorkRequest, where: wr.site_id in ^scope_map.site_ids and wr.location_id in ^scope_map.location_ids and ^from_dt <= wr.raised_date_time and ^to_dt >= wr.raised_date_time)
      |> Repo.all(prefix: prefix)

    count_of_tickets_with_status_closed =
      Enum.count(ticket_list, fn wr -> wr.status == "CS" end)
  ...
  def ticket_tat_response(contract_id, from_date, to_date, prefix) do
    ...
    ticket_list =
      from(wr in WorkRequest, where: wr.site_id in ^scope_map.site_ids and wr.location_id in ^scope_map.location_ids and ^from_dt <= wr.raised_date_time and ^to_dt >= wr.raised_date_time,
      join: wrs in WorkrequestSubcategory, on: wrs.id == wr.workrequest_subcategory_id,
      select: %{
        wr: wr,
        wrs: wrs
      })
  ...
  def ticket_reopen(contract_id, from_date, to_date, prefix) do
    ...
    count_of_ticket_reopen_status =
      from(wr in WorkRequest, where: wr.site_id in ^scope_map.site_ids and wr.location_id in ^scope_map.location_ids and ^from_dt <= wr.raised_date_time and ^to_dt >= wr.raised_date_time,
      join: wrst in WorkrequestStatusTrack, on: wrst.work_request_id == wr.id, where: wrst.status == "ROP",
      select: wrst)
      |> Repo.all(prefix: prefix)
