defmodule Inconn2Service.IotService.Asset do
  ...
  def list_assets_of_meters_and_sensors(site_id, prefix) do
    ...
    locations = AssetConfig.list_locations_by_ids(asset_ids_map["location_ids"], prefix) |> Enum.map(&(add_asset_type_to_asset(&1, "L"))) |> Enum.map(&(preload_parent(&1, &1.asset_type, prefix)))
    equipments = AssetConfig.list_equipments_by_ids(asset_ids_map["equipment_ids"], prefix) |> Enum.map(&(add_asset_type_to_asset(&1, "E"))) |> Enum.map(&(preload_parent(&1, &1.asset_type, prefix)))
