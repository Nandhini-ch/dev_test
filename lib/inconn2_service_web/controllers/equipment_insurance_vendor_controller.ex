defmodule Inconn2ServiceWeb.EquipmentInsuranceVendorController do
  ...
  def index(conn, _params) do
    equipment_insurance_vendors = AssetInfo.list_equipment_insurance_vendors()
  ...
  def create(conn, %{"equipment_insurance_vendor" => equipment_insurance_vendor_params}) do
    with {:ok, %EquipmentInsuranceVendor{} = equipment_insurance_vendor} <- AssetInfo.create_equipment_insurance_vendor(equipment_insurance_vendor_params) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    equipment_insurance_vendor = AssetInfo.get_equipment_insurance_vendor!(id)
  ...
  def update(conn, %{"id" => id, "equipment_insurance_vendor" => equipment_insurance_vendor_params}) do
    ...
    with {:ok, %EquipmentInsuranceVendor{} = equipment_insurance_vendor} <- AssetInfo.update_equipment_insurance_vendor(equipment_insurance_vendor, equipment_insurance_vendor_params) do
      render(conn, "show.json", equipment_insurance_vendor: equipment_insurance_vendor)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %EquipmentInsuranceVendor{}} <- AssetInfo.delete_equipment_insurance_vendor(equipment_insurance_vendor) do
