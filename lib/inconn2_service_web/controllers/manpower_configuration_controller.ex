defmodule Inconn2ServiceWeb.ManpowerConfigurationController do
  ...
  def update(conn, %{"id" => id, "manpower_configuration" => manpower_configuration_params}) do
    manpower_configuration = ContractManagement.get_manpower_configuration!(id, conn.assigns.sub_domain_prefix)

    with {:ok, %ManpowerConfiguration{} = manpower_configuration} <- ContractManagement.update_manpower_configuration(manpower_configuration, manpower_configuration_params, conn.assigns.sub_domain_prefix) do
      render(conn, "show.json", manpower_configuration: manpower_configuration)
    end
  end
