defmodule Inconn2ServiceWeb.AssetStatusTrackController do
  ...
  def index(conn, _params) do
    asset_status_tracks = AssetConfig.list_asset_status_tracks()
  ...
  def create(conn, %{"asset_status_track" => asset_status_track_params}) do
    with {:ok, %AssetStatusTrack{} = asset_status_track} <- AssetConfig.create_asset_status_track(asset_status_track_params) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    asset_status_track = AssetConfig.get_asset_status_track!(id)
  ...
  def update(conn, %{"id" => id, "asset_status_track" => asset_status_track_params}) do
    ...
    with {:ok, %AssetStatusTrack{} = asset_status_track} <- AssetConfig.update_asset_status_track(asset_status_track, asset_status_track_params) do
      render(conn, "show.json", asset_status_track: asset_status_track)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %AssetStatusTrack{}} <- AssetConfig.delete_asset_status_track(asset_status_track) do
