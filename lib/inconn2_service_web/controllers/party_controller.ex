defmodule Inconn2ServiceWeb.PartyController do
  ...
  def index(conn, _params) do
    parties = AssetConfig.list_parties(conn.query_params, conn.assigns.sub_domain_prefix)
    render(conn, "index.json", parties: parties)
  end

  def create(conn, %{"party" => party_params}) do
    with {:ok, %Party{} = party} <- AssetConfig.create_party(party_params, conn.assigns.sub_domain_prefix) do
      conn
      |> put_status(:created)
