defmodule Inconn2ServiceWeb.EquipmentDlpVendorController do
  ...
  def index(conn, _params) do
    equipment_dlp_vendors = AssetInfo.list_equipment_dlp_vendors()
  ...
  def create(conn, %{"equipment_dlp_vendor" => equipment_dlp_vendor_params}) do
    with {:ok, %EquipmentDlpVendor{} = equipment_dlp_vendor} <- AssetInfo.create_equipment_dlp_vendor(equipment_dlp_vendor_params) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    equipment_dlp_vendor = AssetInfo.get_equipment_dlp_vendor!(id)
  ...
  def update(conn, %{"id" => id, "equipment_dlp_vendor" => equipment_dlp_vendor_params}) do
    ...
    with {:ok, %EquipmentDlpVendor{} = equipment_dlp_vendor} <- AssetInfo.update_equipment_dlp_vendor(equipment_dlp_vendor, equipment_dlp_vendor_params) do
      render(conn, "show.json", equipment_dlp_vendor: equipment_dlp_vendor)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %EquipmentDlpVendor{}} <- AssetInfo.delete_equipment_dlp_vendor(equipment_dlp_vendor) do
